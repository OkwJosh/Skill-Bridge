...
class SchoolCreateView(APIView):
    ...
    Lets a freshly-signed-up school admin create their school record.
    The caller is automatically added to `School.admins`. Returns 409
    if the caller already administers a school (one school per admin
    ...
    def post(self, request):
        ...
        if School.objects.filter(admins=request.user).exists():
            return Response(
                {'status': 'error', 'data': None,
                 'meta': {'http_status': 409},
                 'errors': [{'field': None,
                             'message': 'You already administer a school.',
                             'code': 'already_admin'}]},
        ...
        base_slug = slugify(name)[:240] or 'school'
        slug = base_slug
        n = 2
        ...
        with transaction.atomic():
            school = School.objects.create(slug=slug, **serializer.validated_data)
            school.admins.add(request.user)
...
class SchoolRosterCSVImportView(APIView):
    """
    POST /api/v1/schools/me/roster/import/   (multipart/form-data)

    Bulk-imports roster rows from a CSV.

    Required header column: matriculation_number
    Optional columns: email, full_name, department, course_of_study,
        enrollment_year, expected_graduation_year, graduation_year,
        is_graduated, cgpa

    Query params:
      mode=upsert  → existing rows (matched by matric number) are updated
      mode=skip    → existing rows are left untouched (DEFAULT)
    ...
    INT_FIELDS = {'enrollment_year', 'expected_graduation_year', 'graduation_year'}
    BOOL_FIELDS = {'is_graduated'}
    STR_FIELDS = {'email', 'full_name', 'department', 'course_of_study'}

    def _get_admin_school(self, user):
        """Same pattern as SchoolRosterView. Returns (school, err_response)."""
        if not user.is_school_admin:
            return None, Response({
                'status': 'error', 'data': None,
                'meta': {'http_status': 403},
    ...
    def post(self, request):
        school, err = self._get_admin_school(request.user)
        if err:
            return err

        ser = RosterCSVUploadSerializer(data=request.data)
        ser.is_valid(raise_exception=True)
        upload = ser.validated_data['file']
        mode = request.query_params.get('mode', 'skip').lower()
        if mode not in ('skip', 'upsert'):
            mode = 'skip'

        # Decode — accept UTF-8 with optional BOM.
        import csv
        ...
        if not reader.fieldnames or 'matriculation_number' not in [
            ...
            return Response({
                ...
                'meta': {'http_status': 400},
                'errors': [{'field': 'file',
                            'message': 'CSV must include a "matriculation_number" header.',
                            'code': 'missing_header'}],
            }, status=status.HTTP_400_BAD_REQUEST)
        ...
        for row in reader:
            row_num += 1
            matric_raw = (row.get(header_map.get('matriculation_number', ''), '') or '').strip()
            if not matric_raw:
                errors.append({'row': row_num, 'message': 'Missing matriculation_number.'})
                continue

            fields = {}
            for col in self.STR_FIELDS:
                fields[col] = (row.get(header_map.get(col, ''), '') or '').strip()
            for col in self.INT_FIELDS:
                raw_val = (row.get(header_map.get(col, ''), '') or '').strip()
                if raw_val:
                    try:
                        fields[col] = int(raw_val)
                    except ValueError:
                        errors.append({'row': row_num,
                                       'message': f'{col} must be an integer.'})
                        fields[col] = None
                else:
                    fields[col] = None
            for col in self.BOOL_FIELDS:
                raw_val = (row.get(header_map.get(col, ''), '') or '').strip().lower()
                fields[col] = raw_val in ('1', 'true', 'yes', 'y')
            cgpa_raw = (row.get(header_map.get('cgpa', ''), '') or '').strip()
            if cgpa_raw:
                try:
                    fields['cgpa'] = float(cgpa_raw)
                except ValueError:
                    errors.append({'row': row_num, 'message': 'cgpa must be a number.'})
                    fields['cgpa'] = None
            else:
                fields['cgpa'] = None

            existing = StudentRosterRecord.objects.filter(
                school=school, matriculation_number=matric_raw,
            ).first()
            if existing:
                if mode == 'upsert':
            ...
            try:
                StudentRosterRecord.objects.create(
                    school=school,
                    matriculation_number=matric_raw,
                    **fields,
                )
                created += 1
